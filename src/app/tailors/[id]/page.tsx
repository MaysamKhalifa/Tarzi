...
import { MapPin, Star, Clock, Phone, MessageCircle, Heart, ChevronLeft, Scissors, Check } from 'lucide-react'
...
export default function TailorProfilePage({ params }: { params: Promise<{ id: string }> }) {
  ...
  useEffect(() => {
    ...
    supabase.from('saved_tailors').select('id').eq('user_id', user.id).eq('tailor_id', id).single()
    ...
    <div className="min-h-dvh bg-white pb-8">
      ...
      <div className="relative h-52"
        ...
        <button onClick={toggleSave}
          className="absolute top-12 right-4 w-10 h-10 rounded-xl bg-white/20 flex items-center justify-center">
          <Heart size={20} color="white" fill={saved ? 'white' : 'none'} />
        </button>

        <div className="absolute -bottom-10 left-1/2 -translate-x-1/2">
          <div className="w-20 h-20 rounded-2xl border-4 border-white flex items-center justify-center"
            style={{ background: 'linear-gradient(135deg, #fce4ec 0%, #f8bbd0 100)', boxShadow: '0 4px 16px rgba(0,0,0,0.12)' }}>
      ...
      <div className="fixed bottom-0 left-1/2 -translate-x-1/2 w-full max-w-[430px] px-5 py-4 bg-white"
        ...
        <div className="flex gap-3">
          ...
          <Link href={`/booking/alterations?tailorId=${tailor.id}&tailorName=${encodeURIComponent(tailor.name)}`}
